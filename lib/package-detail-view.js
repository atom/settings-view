...
export default class PackageDetailView {
  ...
  completeInitialzation () {
    this.packageCard = this.refs.packageCard ? this.refs.packageCard.packageCard : null
    if (!this.packageCard) { // Had to load this from the network
  ...
  destroy () {
    ...
    if (this.keymapView) {
      // TODO: change to destroy after porting to etch
      this.keymapView.remove()
      this.keymapView = null
    }

    if (this.grammarsView) {
      // TODO: change to destroy after porting to etch
      this.grammarsView.remove()
      this.grammarsView = null
    }

    if (this.snippetsView) {
      // TODO: change to destroy after porting to etch
      this.snippetsView.remove()
      this.snippetsView = null
    }

    if (this.readmeView) {
      // TODO: replace with destroy after porting to etch
  ...
  updateInstalledState () {
    ...
    if (atom.packages.isPackageLoaded(this.pack.name)) {
      if (!atom.packages.isPackageDisabled(this.pack.name)) {
        ...
        this.refs.startupTime.textContent =
