...
function sortSettings (namespace, settings) {
  return Object.keys(settings)
    ...
    .sort((a, b) => {
        const aSchema = atom.config.getSchema(`${namespace}.${a}`)
        const bSchema = atom.config.getSchema(`${namespace}.${b}`)
        if (aSchema && bSchema) {
            return aSchema.order - bSchema.order
        } else if (aSchema) {
            return -1
        } else {
            return 1
...
function elementForSetting (namespace, name, value) {
  ...
  } else if (typeof value === 'boolean' || (schema && schema.type === 'boolean')) {
    controls.appendChild(elementForCheckbox(namespace, name, value))
  } else if (typeof value === 'array' || (schema && schema.type === 'array')) {
    if (isEditableArray(value)) {
      controls.appendChild(elementForArray(namespace, name, value))
    }
  } else if (typeof value === 'object' || (schema && schema.type === 'object')) {
