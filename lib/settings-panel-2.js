...
const SCOPED_SETTINGS = const scopedSettings = [
  'autoIndent',
  'autoIndentOnPaste',
  'invisibles',
  'nonWordCharacters',
...
export default class SettingsPanel extends CollapsibleSectionPanel {
  ...
  appendSettings (namespace, settings) {
    ...
    for (const name of sortedSettings) {
      body.appendChild(appendSetting(namespace, name, settings[name]))
    }
  ...
  bindSelectFields () {
    const disposables = Array.from(this.querySelectorAll('select[id]')).map((select) => {
...
function appendSetting (namespace, name, value) {
  if (namespace === 'core') {
    if (name === 'themes') { return } // Handled in the Themes panel
    if (name === 'disabledPackages') { return } // Handled in the Packages panel
    if (name === 'customFileTypes') { return }
  }
...
function elementForObject (namespace, name, value) {
  if (_.keys(value).length === 0) {
    ...
    for (const key of sortSettings(keyPath, value)) {
      div.appendChild(appendSetting(namespace, `${name}.${key}`, value[key]))
    }
