...
export default class PackageCard {
  ...
  handleButtonEvents (options) {
    ...
    const packageNameClickHandler = (event) => {
      ...
      shell.openExternal(`https://pulsar-edit.com/${packageType}/${this.pack.name}`)
    }
    ...
    const packageAuthorClickHandler = (event) => {
      event.stopPropagation()
      shell.openExternal(`https://pulsar-edit.com/users/${ownerFromRepository(this.pack.repository)}`)
    }
